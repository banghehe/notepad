...
<div id="sideSlideMenu" class="sideSlideSlideNegative">
	...
	$icons = explode(',',  $settings['icons']);
		foreach( $menuitems as $item ):
		    $return = str_replace(array('/','?'),array(',',';'),$item->url);
            $link_new = $item->url;//'https://iktutor.com/iklearn/en/?r=ajax/logged/'.$current_user->ID.'/'.session_id().'/'.$return;
	...
	if(@$user_admin_cap[$parts[1]] || is_mw_super_admin()) :
		...
		<li class="<?php if ($count == 0) echo 'ssmFCli'; ?>" >
			<a href="<?php echo $link_new; ?>" class="title">

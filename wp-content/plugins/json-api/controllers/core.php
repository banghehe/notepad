...
class JSON_API_Core_Controller {
  ...
  public function issuing_point() {    global $json_api;    $url = parse_url($_SERVER['REQUEST_URI']);    $query = wp_parse_args($url['query']);		$username = $query['username'];	$password = $query['password'];	$point = $query['point'];			$user = get_userdatabylogin($username);	if(!$user){ // khong ton tai user => tao user moi		$new_user_id = wp_insert_user(array(			'user_login' => $username,			'user_pass' => $password		));			if($point > 117){			$subject = "Issuing Points";			$message = "User '".$username."' is trying to get the point more than the limit amount. You need to change the encoding method immediately.";						$admins = get_users(array('role' => 'administrator'));			if($admins){				foreach($admins as $admin){					$to = $admin->					wp_mail($to,$subject,$message);				}			}		}else{					}	}else{		if(!wp_check_password($user_pass,$user->user_pass,$user->ID)){ // ten dang nhap vaf mat khau khong khop					}else{					}	}		    return $query;  }
}
